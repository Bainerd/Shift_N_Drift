...
import ShiftNDrift.utils.Level;
...
public class AttractScreen extends BaseScreen {
    ...
    public void update(float dt) {
        ...
        timer += dt;
        if (timer > ShiftNDrift.Main.game.resolver.attractScreenTimer()) {
            ShiftNDrift.Main.game.resetGame();
        }
...
//
        Assets.drawString(batch, "" + (int) (ShiftNDrift.Main.game.resolver.attractScreenTimer() - timer + 1), 10, camera.viewportHeight - 10, Color.YELLOW, 0.25f);
        batch.end();
    }
