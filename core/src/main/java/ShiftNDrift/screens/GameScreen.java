...
import ShiftNDrift.ParticleSystem.WindParticle;
import ShiftNDrift.backgroundobjects.Bird;
import ShiftNDrift.backgroundobjects.Cloud;
import ShiftNDrift.backgroundobjects.HotairBalloon;
import ShiftNDrift.backgroundobjects.Ufo;
import ShiftNDrift.gameobjects.*;
import ShiftNDrift.ui.Button;
import ShiftNDrift.ui.StateButton;
import ShiftNDrift.utils.*;
import ShiftNDrift.utils.accessors.CameraAccessor;
import ShiftNDrift.utils.accessors.ColorAccessor;
import ShiftNDrift.utils.accessors.Vector2Accessor;
...
public class GameScreen extends BaseScreen {
    ...
    public void update(float dt) {
        ...
        if (playerBalloon.currentState == Balloon.State.DEAD) {
            continueTimer += dt;
            if (continueTimer > ShiftNDrift.Main.game.resolver.continueTimer()){
                ShiftNDrift.Main.game.resetGame();
            }
        }
        ...
        if (timeoutDelay > ShiftNDrift.Main.game.resolver.limitTimer() && playerBalloon.currentState != Balloon.State.DEAD){
            setShowContinue();
        }

        if (Gdx.input.isKeyJustPressed(Input.Keys.ESCAPE)) {
            ShiftNDrift.Main.game.screen = new LevelSelectScreen(level.levelIndex);
        }
...
//        }
        ...
        int timeoutLeft = (int)(ShiftNDrift.Main.game.resolver.limitTimer() - timeoutDelay) + 1;
        if (timeoutLeft < ShiftNDrift.Main.game.resolver.warningTimer() && !drawStats){
        ...
        if (playerBalloon.currentState == Balloon.State.DEAD) {
            ...
            int timeLeft = (int)(ShiftNDrift.Main.game.resolver.continueTimer() - continueTimer) + 1;
        ...
        for (String text : statsText) {
            ...
            // Everything else
                ...
                switch (i) {
                    ...
                    case 5: {
                        long millis = Statistics.endTime - ShiftNDrift.Main.startTime;
