...
public class LevelSelectScreen extends BaseScreen {
    ...
    public void update(float dt) {
        if (Gdx.input.isKeyJustPressed(Input.Keys.ESCAPE)) {
            ShiftNDrift.Main.game.screen = new MenuScreen();
        }
        timeoutTimer += dt;
        if (Gdx.input.justTouched()){
            timeoutTimer = 0;
        }
        if (timeoutTimer > ShiftNDrift.Main.game.resolver.limitTimer()){
    ...
    public boolean touchUp(int screenX, int screenY, int pointer, int button) {
        ...
        for (LevelButton levelButton : buttons) {
            if (levelButton.active && levelButton.checkForTouch(touchPosScreen.x, touchPosScreen.y)) {
                ShiftNDrift.Main.game.screen = new GameScreen(levelButton.levelId);
            }
        }
