...
public class MenuScreen extends BaseScreen {
    ...
    public void update(float dt) {
        ...
        if (Gdx.input.justTouched()) {
            ShiftNDrift.Main.game.screen = new LevelSelectScreen(0);
        }

        color.set(1f, 1f, 1f, alpha.floatValue());

        timer += dt;
        if (timer > ShiftNDrift.Main.game.resolver.menuScreenTimer()) {
            ShiftNDrift.Main.game.screen = new AttractScreen();
        }
    }
    ...
    public void render(SpriteBatch batch) {
        ...
        Assets.drawString(batch, "" + (int) (ShiftNDrift.Main.game.resolver.menuScreenTimer() - timer + 1), 10, camera.viewportHeight - 10, Color.YELLOW, 0.25f);
        batch.end();
    }
